...
class ProductService:
    ...
    async def create(self, data: ProductCreate) -> Product:
        await self.categories.ensure_exists(data.category_id)

        if data.image_id is not None:
            await self.images.ensure_exists(data.image_id)
    ...
    async def update(self, product_id: int, data: ProductUpdate) -> Product:
        ...
        if "category_id" in update_fields:
            ...
            if new_category_id != db_product.category_id:
                await self.categories.ensure_exists(new_category_id)
        ...
        if "image_id" in update_fields:
            ...
            if new_image_id != db_product.image_id:
                if new_image_id is not None:
                    await self.images.ensure_exists(new_image_id)
    ...
    async def delete(self, product_id: int) -> None:
        db_product = await self.db.get(ProductSchema, product_id)
    ...
    async def _resolve_category_ids(self, category_name: str) -> list[int]:
        """Ids of every category matching this name (case-insensitive; more
        than one can match, since names are only unique per parent) plus all
        of their descendants, found via a recursive CTE walking
        parent_id -> id."""
        roots = (
    ...
    async def search(
        ...
        *,
        ...
        if category:
            category_ids = await self._resolve_category_ids(category)
        ...
        db_products = (await self.db.scalars(stmt)).all()
